...
#include <vector>
#include <string>
#include <HookLoadAntiCollisionDetection/CSF/point_cloud.h>
#include <HookLoadAntiCollisionDetection/CSF/Cloth.h>
...
public:
    ...
    void do_filtering_v2(std::vector<int>& groundIndexes,
                        std::vector<int>& offGroundIndexes,
                        bool exportCloth);
    std::vector<double> do_cloth_export();

    pcl::PointCloud<pcl::PointXYZ>::Ptr point_cloud_XYZ_ = boost::make_shared<pcl::PointCloud<pcl::PointXYZ>>();
    pcl::PointCloud<pcl::PointXYZI>::Ptr point_cloud_XYZI_ = boost::make_shared<pcl::PointCloud<pcl::PointXYZI>>();

        // 初始化最小最大点
    csf::Point bbMin  = csf::Point(std::numeric_limits<float>::max(),std::numeric_limits<float>::max(),std::numeric_limits<float>::max());
    csf::Point bbMax  = csf::Point(std::numeric_limits<float>::lowest(),std::numeric_limits<float>::lowest(),std::numeric_limits<float>::lowest());
private:
